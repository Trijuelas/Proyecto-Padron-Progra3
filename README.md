...
tcp.port=5000
http.port=8080
padron.path=datos/PADRON_COMPLETO.txt
distritos.path=datos/distelec.txt
worker.threads=12
```
...
```powershell
java -cp build/classes padron.App --padron.path='datos/PADRON_COMPLETO.txt' --distritos.path='datos/distelec.txt'
```
...
Los TXT originales se leen en modo consulta y nunca se modifican. Para esta entrega se ubican en `datos/`; `PADRON_COMPLETO.txt` se administra con Git LFS por su tamano.
